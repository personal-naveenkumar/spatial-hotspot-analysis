...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = 
  {
    ...
    val joinCountOutput = spark.sql("select rectangle, count(point) as count from joinResult group by rectangle order by rectangle")
    joinCountOutput.createOrReplaceTempView("joinCountOutput")

    return joinCountOutput
  }
